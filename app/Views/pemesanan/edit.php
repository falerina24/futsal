...
<div class="container mt-5">
    ...
    <form action="" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" name="nomor" class="form-control" value="<?= $pemesanan['nomor']; ?>" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select name="pembayaran_id" class="form-control">
                <?php foreach ($pembayaran as $item) : ?>
                    <option value="<?= $item['id'] ?>" <?= isset($pemesanan['pembayaran_id']) && $pemesanan['pembayaran_id'] == $item['id'] ? 'selected' : ''; ?>><?= $item['id'] ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="form-group">
            ...
            <select name="jadwal_id" id="jadwal_id" class="form-control">
                <?php foreach ($jadwal as $item) : ?>
                    <option value="<?= $item['id'] ?>" <?= isset($pemesanan['jadwal_id']) && $pemesanan['jadwal_id'] == $item['id'] ? 'selected' : ''; ?>><?= $item['id'] ?></option>
                <?php endforeach; ?>
            </select>
        </div>
